<template>
    ...
    <div class="card">
      ...
      <div class="overflow-x-auto">
        <table class="w-full">
          ...
          <tbody>
            <tr v-for="(user, idx) in pagedUsers" :key="idx" class="border-b border-gray-100 hover:bg-gray-50 transition">
              <td class="py-4 px-4">
              ...
              <td class="py-4 px-4">
                <div class="flex gap-2">
                  <button @click="editUser(user)" class="p-1 hover:bg-gray-200 rounded transition">
                    <Edit2 class="w-4 h-4 text-gray-600" />
                  </button>
                  <button @click="deleteUser(user)" class="p-1 hover:bg-red-100 rounded transition">
      ...
      <div class="flex justify-between items-center mt-6 pt-4 border-t border-gray-200">
        <p class="text-sm text-gray-600">Showing {{ pageStart }} to {{ pageEnd }} of {{ filteredUsers.length }} users</p>
        <div class="flex gap-2">
          <button @click="prevPage" :disabled="page === 1" class="px-3 py-1 border border-gray-300 rounded-lg hover:bg-gray-50 transition text-sm disabled:opacity-50">Previous</button>
          <button @click="nextPage" :disabled="page === totalPages" class="px-3 py-1 border border-gray-300 rounded-lg hover:bg-gray-50 transition text-sm disabled:opacity-50">Next</button>
        </div>
      </div>
    </div>
  ...
  <!-- Feedbacks -->
  <div id="feedbacks" class="card">
      <div class="flex justify-between items-center mb-4">
        <h3 class="text-lg font-semibold">Recent Feedback</h3>
        <p class="text-sm text-gray-500">Latest messages from users</p>
      </div>
      <div v-if="loadingFeedbacks" class="py-6 text-center text-gray-500">Loading...</div>
      <div v-else>
        <div v-if="feedbacks.length === 0" class="py-6 text-gray-600">No feedbacks yet.</div>
        <div v-else class="space-y-3">
          <div v-for="f in feedbacks.slice(0,6)" :key="f.feedback_id" class="p-3 border rounded-lg">
            <div class="flex justify-between items-start">
              <div>
                <p class="font-semibold">{{ f.name || 'Anonymous' }} <span class="text-xs text-gray-500">— {{ f.email }}</span></p>
                <p class="text-sm text-gray-700 mt-1">{{ f.message }}</p>
              </div>
              <div class="text-xs text-gray-400">{{ new Date(f.created_at).toLocaleString() }}</div>
            </div>
          </div>
        </div>
      </div>
...
const fetchRecentUsers = async () => {
  try {
    ...
    const data = res.data || []
    users.value = (data || []).map(u => ({ name: u.full_name || u.full_name, email: u.email, role: u.role, organization: u.organization || '-', active: u.is_active }))
  } catch (e) {
    // keep default or empty
  }
}
...
const loadFeedbacks = async () => {
  ...
  try {
    ...
    // if there are items and localStorage says none unread, mark unread
    try {
      const prevSeen = localStorage.getItem('feedback-last-count') || '0'
      const prevNum = parseInt(prevSeen || '0', 10)
      if (newList.length > prevNum) {
        // mark unread
        localStorage.setItem('feedback-unread', '1')
        window.dispatchEvent(new CustomEvent('feedback-new'))
      }
      localStorage.setItem('feedback-last-count', String(newList.length))
    } catch (e) {}
    feedbacks.value = newList
  } catch (e) {
    feedbacks.value = []
  } finally {
...
const pageEnd = computed(() => Math.min(filteredUsers.value.length, page.value * perPage.value))

const prevPage = () => { if (page.value > 1) page.value-- }
const nextPage = () => { if (page.value < totalPages.value) page.value++ }

// clamp page when filters or list change
watch([() => activeFilter.value, () => users.value.length], () => {
...
const editUser = (user) => {
  // show an edit modal or navigate to edit page (simple alert for now)
  // TODO: Replace with modal form that calls API
...
const deleteUser = (user) => {
  ...
  const idx = users.value.findIndex(u => u.email === user.email)
  if (idx !== -1) users.value.splice(idx, 1)
}

const toggleActive = (user) => {
  // flip active state and optionally call backend
  user.active = !user.active
  // TODO: call API to persist change
}

// no duplicate onMounted

// scroll to feedbacks if hash present
import { useRoute } from 'vue-router'
const routeLocal = useRoute()
watch(() => routeLocal.hash, (h) => {
  if (h === '#feedbacks') {
    setTimeout(() => {
