...
-- Feedbacks (user-submitted feedback / contact messages)
CREATE TABLE IF NOT EXISTS feedbacks (
  feedback_id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  name VARCHAR(255),
  email VARCHAR(255) NOT NULL,
...
INSERT INTO feedbacks (feedback_id, name, email, phone, message)
VALUES
  ('f1','Alice Example','alice@example.com','+14045550001','Great product! The e-prescription workflow saved us a lot of time.'),
  ('f2','Bob Tester','bob@example.com','+14045550002','I found a minor UI issue on mobile when booking appointments.')
ON CONFLICT DO NOTHING;
